...
- (id)initWithURL:(NSURL*)pageURL {
    ...
    if(self = [super init]) {
        if([pageURL.absoluteString hasPrefix:@"http"]||[[pageURL.absoluteString uppercaseString] hasPrefix:@"HTTP"])
        {
            
        }
        else
        {
            NSString *urlS = [@"http://" stringByAppendingString:pageURL.absoluteString];
            NSString *encodedString=[urlS stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
            pageURL = [NSURL URLWithString:encodedString];
        }
        self.URL = pageURL;
    }
