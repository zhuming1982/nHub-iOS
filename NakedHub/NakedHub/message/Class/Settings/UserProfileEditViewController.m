...
//        UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:NSLocalizedString(@"cancel", @"Cancel") destructiveButtonTitle:nil otherButtonTitles:NSLocalizedString(@"setting.cameraUpload", @"Take photo"),NSLocalizedString(@"setting.localUpload", @"Photos"), nil];
         ...
         UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:[GDLocalizableClass getStringForKey:@"cancel"]
                destructiveButtonTitle:nil otherButtonTitles:[GDLocalizableClass getStringForKey:@"setting.cameraUpload"],
            [GDLocalizableClass getStringForKey:@"setting.localUpload"], nil];
        
        [actionSheet showInView:[[UIApplication sharedApplication] keyWindow]];
    } else if (indexPath.row == 1) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:[GDLocalizableClass getStringForKey:@"setting.editName"]delegate:self cancelButtonTitle:[GDLocalizableClass getStringForKey:@"cancel"] otherButtonTitles:[GDLocalizableClass getStringForKey:@"ok"], nil];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if ([alertView cancelButtonIndex] != buttonIndex) {
        ...
        {
            //设置推送设置
            [self showHint:[GDLocalizableClass getStringForKey:@"setting.saving"]];
            ...
            [[UserProfileManager sharedInstance] updateUserProfileInBackground:@{kPARSE_HXUSER_NICKNAME:nameTextField.text} completion:^(BOOL success, NSError *error) {
                ...
                if (success) {
                    ...
                    [self showHint:[GDLocalizableClass getStringForKey:@"setting.saveFailed"] yOffset:0];
                }
            }];
        }
    }
...
{
    [self hideHud];
    [self showHudInView:self.view hint:[GDLocalizableClass getStringForKey:@"setting.uploading"]];
    ...
    if (orgImage) {
        [[UserProfileManager sharedInstance] uploadUserHeadImageProfileInBackground:orgImage completion:^(BOOL success, NSError *error) {
            ...
            if (success) {
                ...
                [self showHint:[GDLocalizableClass getStringForKey:@"setting.uploadSuccess"]];
            } else {
                [self showHint:[GDLocalizableClass getStringForKey:@"setting.uploadFail"]];
            }
        }];
    } else {
        [self hideHud];
