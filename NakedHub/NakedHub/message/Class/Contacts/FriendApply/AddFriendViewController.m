...
- (void)viewDidLoad
{
    ...
    self.title = [GDLocalizableClass getStringForKey:@"friend.add"];
    ...
    [searchButton setTitle:[GDLocalizableClass getStringForKey:@"search"] forState:UIControlStateNormal];
...
- (UITextField *)textField
{
    if (_textField == nil) {
        ...
        _textField.placeholder = [GDLocalizableClass getStringForKey:@"friend.inputNameToSearch"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self didBuddyExist:buddyName]) {
        NSString *message = [NSString stringWithFormat:[GDLocalizableClass getStringForKey:@"friend.repeat" withComment:@"'%@'has been your friend!"], buddyName];
        
        [EMAlertView showAlertWithTitle:message
                                message:nil
                        completionBlock:nil
                      cancelButtonTitle:[GDLocalizableClass getStringForKey:@"ok"]
                      otherButtonTitles:nil];
        
    }
    else if([self hasSendBuddyRequest:buddyName])
    {
        NSString *message = [NSString stringWithFormat:[GDLocalizableClass getStringForKey:@"friend.repeatApply" withComment:@"you have send fridend request to '%@'!"],buddyName];
        [EMAlertView showAlertWithTitle:message
                                message:nil
                        completionBlock:nil
                      cancelButtonTitle:[GDLocalizableClass getStringForKey:@"ok"]
...
#warning 以下代码为测试代码，默认用户体系中有一个符合要求的同名用户
        ...
        if ([_textField.text isEqualToString:loginUsername]) {
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:[GDLocalizableClass getStringForKey:@"prompt"]
 message:[GDLocalizableClass getStringForKey:@"friend.notAddSelf"]
                        delegate:nil cancelButtonTitle:[GDLocalizableClass getStringForKey:@"ok"] otherButtonTitles:nil, nil];
            [alertView show];
        ...
        if (applyArray && [applyArray count] > 0) {
            for (ApplyEntity *entity in applyArray) {
                ...
                if (!isGroup && [entity.applicantUsername isEqualToString:_textField.text]) {
                    NSString *str = [NSString stringWithFormat:[GDLocalizableClass getStringForKey:@"friend.repeatInvite" withComment:@"%@ have sent the application to you"], _textField.text];
                    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:
                                              [GDLocalizableClass getStringForKey:@"prompt"]
                                            message:str delegate:nil cancelButtonTitle:[GDLocalizableClass getStringForKey:@"ok"]  otherButtonTitles:nil, nil];
                    [alertView show];
...
- (void)showMessageAlertView
{
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil
                                        message:[GDLocalizableClass getStringForKey:@"saySomething"]
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if ([alertView cancelButtonIndex] != buttonIndex) {
        ...
        if (messageTextField.text.length > 0) {
            ...
            messageStr = [NSString stringWithFormat: [GDLocalizableClass getStringForKey:@"friend.somebodyInvite" withComment:@"%@ invite you as a friend"], username];
        }
                           ...
                           message:(NSString *)message
{
    ...
    if (buddyName && buddyName.length > 0) {
        [self showHudInView:self.view hint:[GDLocalizableClass getStringForKey:@"friend.sendApply"]];
        EMError *error;
        [[EaseMob sharedInstance].chatManager addBuddy:buddyName message:message error:&error];
        [self hideHud];
        if (error) {
            [self showHint:[GDLocalizableClass getStringForKey:@"friend.sendApplyFail"]];
        }
        else{
            [self showHint:[GDLocalizableClass getStringForKey:@"friend.sendApplySuccess"]];
