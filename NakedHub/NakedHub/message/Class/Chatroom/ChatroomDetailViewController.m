...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        cell.textLabel.text =[GDLocalizableClass getStringForKey:@"chatroom.id"];
        cell.accessoryType = UITableViewCellAccessoryNone;
        cell.detailTextLabel.text = _chatroom.chatroomId;
    }
    else if (indexPath.row == 2)
    {
        cell.textLabel.text = [GDLocalizableClass getStringForKey:@"chatroom.description"];
        cell.accessoryType = UITableViewCellAccessoryNone;
        cell.detailTextLabel.text = _chatroom.chatroomDescription;
    }
    else if (indexPath.row == 3)
    {
        cell.textLabel.text = [GDLocalizableClass getStringForKey:@"chatroom.occupantCount"];
        cell.accessoryType = UITableViewCellAccessoryNone;
...
- (void)fetchChatroomInfo
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(){
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (error)
            {
                [weakSelf showHint:[GDLocalizableClass getStringForKey:@"chatroom.fetchInfoFail"]];
            }
        });
    });
}
