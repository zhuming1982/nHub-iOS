...
@interface GroupListViewController ()<UISearchBarDelegate, UISearchDisplayDelegate, IChatManagerDelegate, SRRefreshDelegate,ChatViewControllerDelegate>
...
//            return cell;
            ...
            static NSString *CellIdentifier = @"GYChatGroupCell";
            GYChatGroupCell *cell = (GYChatGroupCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
            
            // Configure the cell...
            if (cell == nil) {
                cell = [[GYChatGroupCell alloc] init];
            }
            [weakSelf.groups enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if ([((NakedHubGroupModel*)obj).huanxinGroupId isEqualToString:group.groupId]) {
                    cell.groupModel = ((NakedHubGroupModel*)obj);
                    *stop = true;
                }
            }];
...
- (void)reloadDataSource
{
    ...
    [self loadChatGroupAvater];
}

-(void)loadChatGroupAvater
{
    NSArray *rooms = [[EaseMob sharedInstance].chatManager groupList];
    NSString *groupIds= @"";
    if (rooms.count>0) {
        ...
        for (EMGroup *group in rooms) {
            ...
            {
                groupIds = [groupIds stringByAppendingString:[NSString stringWithFormat:@",%@",group.groupId]];
            }
        }
    }
    ...
    if(groupIds.length>0)
    {
//        [HttpRequest getWithUrl:API_IM_CHATGROUP_BATCHQUERY andViewContoller:self andHudMsg:nil andAttributes:[NSMutableDictionary dictionaryWithDictionary:@{@"huanxinGroupIds":groupIds}] andBlock:^(id response, NSError *error) {
//            if (!error) {
//                _groups = [MTLJSONAdapter modelsOfClass:[GYGroupModel class] fromJSONArray:response[@"result"] error:nil];
//                [self.tableView reloadData];
//            }
//        }];
    }
...
//    }
//    for (GYUserModel* user in _userModels) {
//        if (user.id == [chatter integerValue]) {
//            return user.avatar.url_280_280;
//        }
//    }
//    return nil;
//}
