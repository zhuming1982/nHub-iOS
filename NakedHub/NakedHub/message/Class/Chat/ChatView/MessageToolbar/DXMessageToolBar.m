...
- (void)sendFace
{
    [mixPanel track:@"Chat_send" properties:logInDic];
    NSString *chatText = self.inputTextView.text;
    if (chatText.length > 0) {
        if ([self.delegate respondsToSelector:@selector(didSendText:)]) {
...
//    self.inputTextView.contentMode = UIViewContentModeCenter;
    ...
    _inputTextView.placeHolder = [GDLocalizableClass getStringForKey:@"message.toolBar.inputPlaceHolder" withComment: @"input a new message"];
