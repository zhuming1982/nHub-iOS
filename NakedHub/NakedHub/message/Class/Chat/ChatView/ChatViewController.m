...
//#import "GYFriendDetailViewController.h"
...
{
    [self showHudInView:self.view hint:[GDLocalizableClass getStringForKey:@"chatroom.joining"]];
...
//    [self.navigationItem setLeftBarButtonItem:backItem];
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"iconNavBack"] style:UIBarButtonItemStylePlain target:self action:@selector(back)];
...
//        NSString *url = (NSString*)[[NSUserDefaults standardUserDefaults]objectForKey:kUserAvatarUrl];
        [clearButton sd_setImageWithURL:[NSURL URLWithString:_HeadPortrait] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@""]];
...
- (DXMessageToolBar *)chatToolBar
{
    if (_chatToolBar == nil) {
        ...
        _chatToolBar.moreView = [[DXChatBarMoreView alloc] initWithFrame:CGRectMake(0, (kVerticalPadding * 2 + kInputTextViewMinHeight), _chatToolBar.frame.size.width, 108+50) type:type];// 108为显示的map，camera，photo按钮的高度
        _chatToolBar.moreView.backgroundColor = RGBACOLOR(247, 249, 250, 1);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
           ...
           /* if (((MessageModel *)obj).message.ext[@"type"]) {
                if ([((MessageModel *)obj).message.ext[@"type"]isEqualToString:@"ACTIVITY_INVITE"])
                {
                    GYChatReceiverUrlCell *cell = [tableView dequeueReusableCellWithIdentifier:@"GYChatReceiverUrlCell"];
                    if (!cell) {
                        cell = [[GYChatReceiverUrlCell alloc]init];
                    }
                    [cell setSelectEventDetail:^(GYChatReceiverUrlCell *sCell) {
//                        ActivityDetailViewController *detailVC = (ActivityDetailViewController*)[Utility pushViewController:@"ActivityDetail" parent:self];
//                        MessageModel *model = self.dataSource[[tableView indexPathForCell:sCell].row];
//                        detailVC.viewModel.id = [model.message.ext[@"content"][@"id"] integerValue];
                    }];
                    cell.titleLabel.text = model.message.ext[@"content"][@"activityTitle"];
                    cell.eventAdressLabel.text = [NSString stringWithFormat:@"地点:%@",model.message.ext[@"content"][@"sportVenueName"]];
                    cell.eventTimeLabel.text = [NSString stringWithFormat:@"时间:%@",model.message.ext[@"content"][@"holdTime"]];
                    [cell.userAvatarView sd_setImageWithURL:model.headImageURL placeholderImage:[UIImage imageNamed:@"Avatar"]];
            ...
            if (self.conversationType == eConversationTypeGroupChat) {
                if ([model.username integerValue] == [[[NSUserDefaults standardUserDefaults]objectForKey:kUserIdName]integerValue]) {
                    [cell.headImageView sd_setImageWithURL:[NSURL URLWithString:[[NSUserDefaults standardUserDefaults]objectForKey:kUserAvatarUrl]] placeholderImage:[UIImage imageNamed:@"userIcon"]];
                    cell.nameLabel.text = [[NSUserDefaults standardUserDefaults]objectForKey:kUserName];
                }
                else
                {
                    NSMutableArray<NHChatUserModel*> *Arr = [GYSQL selectUserWith:model.username];
                    if (Arr.count) {
                        [cell.headImageView sd_setImageWithURL:[NSURL URLWithString:Arr[0].UserAvatarUrl] placeholderImage:[UIImage imageNamed:@"userIcon"]];
                        cell.nameLabel.text = Arr[0].UserName;
                    }
                    else
                    {
                        [cell.headImageView sd_setImageWithURL:model.message.ext[kUserAvatarUrl] placeholderImage:[UIImage imageNamed:@"userIcon"]];
                        cell.nameLabel.text = model.message.ext[kUserName];
                    }
                }
               /* [_members enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    if (((NakedUserModel*)obj).userId == [model.username integerValue]) {
                        [cell.headImageView sd_setImageWithURL:[NSURL URLWithString:((NakedUserModel*)obj).portait] placeholderImage:[UIImage imageNamed:@"Avatar"]];
            ...
            {
                if ([model.nickName isEqualToString:model.username]) {
                    [cell.headImageView sd_setImageWithURL:model.message.ext[kUserAvatarUrl] placeholderImage:[UIImage imageNamed:@"userIcon"]];
                    cell.nameLabel.text = model.message.ext[kUserName];
                }
            }
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)recognizer
{
    if (recognizer.state == UIGestureRecognizerStateBegan && [self.dataSource count] > 0) {
        ...
        if ([object isKindOfClass:[MessageModel class]]) {
            ...
            if ([[self.tableView cellForRowAtIndexPath:indexPath] isKindOfClass:[EMChatViewCell class]]) {
            ...
            {
                GYChatReceiverUrlCell *cell = (GYChatReceiverUrlCell *)[self.tableView cellForRowAtIndexPath:indexPath];
                [cell becomeFirstResponder];
                _longPressIndexPath = indexPath;
                [self showMenuViewController:cell.messageBgView andIndexPath:indexPath messageType:eMessageBodyType_Text];
            }
...
{
    if (self.conversationType == eConversationTypeGroupChat) {
        [HttpRequest getWithUrl:chat_members andViewContoller:nil andHudMsg:@"" andAttributes:[NSMutableDictionary dictionaryWithDictionary:@{@"huanxinGroupId":self.chatter}] andBlock:^(id response, NSError *error) {
            if (!error) {
                _members = [MTLJSONAdapter modelsOfClass:[NakedUserModel class] fromJSONArray:response[@"result"]  error:nil];
...
{
    [mixPanel track:@"Chat_userDetail" properties:logInDic];
    NakedUserModel *userModel = [[NakedUserModel alloc]init];
    userModel.userId = [model.username integerValue];
...
-(void)chatAudioCellBubblePressed:(MessageModel *)model
{
    ...
    if (downloadStatus == EMAttachmentDownloading) {
        [self showHint:[GDLocalizableClass getStringForKey:@"message.downloadingAudio"]];
        return;
    }
...
-(void)chatLocationCellBubblePressed:(MessageModel *)model
{
    ...
    LocationViewController *locationController = [[LocationViewController alloc] initWithLocation:CLLocationCoordinate2DMake(model.latitude, model.longitude) address:model.address];
...
- (void)chatVideoCellPressed:(MessageModel *)model
{
    ...
    __weak ChatViewController *weakSelf = self;
    id <IChatManager> chatManager = [[EaseMob sharedInstance] chatManager];
    [weakSelf showHudInView:weakSelf.view hint:[GDLocalizableClass getStringForKey:@"message.downloadingVideo"]];
    [chatManager asyncFetchMessage:model.message progress:nil completion:^(EMMessage *aMessage, EMError *error) {
        ...
        if (!error) {
            ...
            [weakSelf showHint:[GDLocalizableClass getStringForKey:@"message.videoFail"]];
        }
    } onQueue:nil];
}
...
-(void)chatImageCellBubblePressed:(MessageModel *)model
{
    ...
    if ([model.messageBody messageBodyType] == eMessageBodyType_Image) {
        ...
        if (imageBody.thumbnailDownloadStatus == EMAttachmentDownloadSuccessed) {
            ...
            [weakSelf showHudInView:weakSelf.view hint:[GDLocalizableClass getStringForKey:@"message.downloadingImage"]];
            [chatManager asyncFetchMessage:model.message progress:nil completion:^(EMMessage *aMessage, EMError *error) {
                [weakSelf hideHud];
                if (!error) {
                    //发送已读回执
                ...
                [weakSelf showHint:[GDLocalizableClass getStringForKey:@"message.imageFail"]];
            } onQueue:nil];
        }else{
            //获取缩略图
            [chatManager asyncFetchMessageThumbnail:model.message progress:nil completion:^(EMMessage *aMessage, EMError *error) {
                if (!error) {
                    ...
                    [weakSelf showHint:[GDLocalizableClass getStringForKey:@"message.thumImageFail"]];
                }
...
-(void)didSendMessage:(EMMessage *)message error:(EMError *)error
{
    ...
    NSMutableDictionary *dic =  [NSMutableDictionary dictionaryWithDictionary:@{@"chatType":message.messageType == eMessageTypeChat ?@"USER2USER":@"USER2GROUP",@"refId":message.conversationChatter,@"easemobMessageId":message.messageId}];
    
    switch (((id<IEMMessageBody>)message.messageBodies[0]).messageBodyType) {
        case eMessageBodyType_Text:
        {
            [dic setObject:@"TEXT" forKey:@"messageType"];
            [dic setObject:((EMTextMessageBody*)message.messageBodies[0]).text forKey:@"content"];
        }
            break;
        case eMessageBodyType_Image:
        {
            [dic setObject:@"PICTURE" forKey:@"messageType"];
        }
            break;
        case eMessageBodyType_Video:
        {
            [dic setObject:@"VIDEO" forKey:@"messageType"];
        }
            break;
        case eMessageBodyType_Location:
        {
            [dic setObject:@"LOCATION" forKey:@"messageType"];
        }
            break;
        case eMessageBodyType_Voice:
        {
            [dic setObject:@"VOICE" forKey:@"messageType"];
        }
            break;
        default:
            break;
    }
    [HttpRequest postWithURLSession:chat_saveChatMessage andViewContoller:nil andAttributes:dic andBlock:^(id response, NSError *error) {
            ...
            if ([object isKindOfClass:[MessageModel class]]) {
                ...
                if ([messageId isEqualToString:currMsg.messageId]) {
                    ...
                    if (error && error.errorCode == EMErrorMessageContainSensitiveWords)
                    {
                        ...
                        [self showHint:[GDLocalizableClass getStringForKey:@"message.forbiddenWords"] yOffset:-frame.size.height + KHintAdjustY];
                    }
                    break;
                }
            }
...
- (void)moreViewTakePicAction:(DXChatBarMoreView *)moreView
{
    ...
    [self showHint:[GDLocalizableClass getStringForKey:@"message.simulatorNotSupportCamera"]];
...
{
    NSDictionary *ext = [self setMessageExt];
...
- (void)didFinishRecoingVoiceAction:(UIView *)recordView
{
    ...
    [[EMCDDeviceManager sharedInstance] asyncStopRecordingWithCompletion:^(NSString *recordPath, NSInteger aDuration, NSError *error) {
        if (!error) {
            ...
            [weakSelf showHudInView:self.view hint:[GDLocalizableClass getStringForKey:@"media.timeShort"]];
    ...
    if ([messagesArray count] > 0) {
        for (EMMessage *message in messagesArray) {
            ...
            if (tempDate > 60 || tempDate < -60 || (self.chatTagDate == nil)) {
                [formatArray addObject:[createDate timeFormatted]];
...
-(NSMutableArray *)formatMessage:(EMMessage *)message
{
    ...
    NSTimeInterval tempDate = [createDate timeIntervalSinceDate:self.chatTagDate];
    if (tempDate > 60 || tempDate < -60 || (self.chatTagDate == nil)) {
        [ret addObject:[createDate timeFormatted]];
        self.chatTagDate = createDate;
    }
...
- (void)removeAllMessages:(id)sender
{
    if (_dataSource.count == 0) {
        [self showHint:[GDLocalizableClass getStringForKey:@"message.noMessage"]];
        return;
    }
...
- (void)showMenuViewController:(UIView *)showInView andIndexPath:(NSIndexPath *)indexPath messageType:(MessageBodyType)messageType
{
    ...
    if (_copyMenuItem == nil) {
        _copyMenuItem = [[UIMenuItem alloc] initWithTitle:[GDLocalizableClass getStringForKey:@"copy"] action:@selector(copyMenuAction:)];
    }
    if (_deleteMenuItem == nil) {
        _deleteMenuItem = [[UIMenuItem alloc] initWithTitle:[GDLocalizableClass getStringForKey:@"delete"] action:@selector(deleteMenuAction:)];
    }
...
- (NSDictionary *)setMessageExt
{
    return @{kUserName:[[NSUserDefaults standardUserDefaults]objectForKey:kUserName],kUserAvatarUrl:[[NSUserDefaults standardUserDefaults]objectForKey:kUserAvatarUrl],kUserIdName:[[[NSUserDefaults standardUserDefaults]objectForKey:kUserIdName] stringValue]};
}

-(void)sendTextMessage:(NSString *)textMessage
{
    if ([Utility isEmpty:textMessage]) {
