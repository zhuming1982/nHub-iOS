...
// 要提前设置视频通话对方图像的显示区域
    ...
    if (_isIncoming) {
        _statusLabel.text = [GDLocalizableClass getStringForKey:@"call.waiting"] ;
        [_actionView addSubview:_answerButton];
        [_actionView addSubview:_rejectButton];
    }
    else{
        _statusLabel.text = [GDLocalizableClass getStringForKey:@"call.connecting"] ;
...
- (void)_setupSubviews
{
    ...
    _headerImageView.image = [UIImage imageNamed:@"Avatar"];
    ...
    _silenceLabel.text =[GDLocalizableClass getStringForKey:@"call.silence"];
    ...
    _speakerOutLabel.text = [GDLocalizableClass getStringForKey:@"call.speaker"];
    
    _rejectButton = [[UIButton alloc] initWithFrame:CGRectMake((tmpWidth - 100) / 2, CGRectGetMaxY(_speakerOutLabel.frame) + 30, 100, 40)];
    [_rejectButton setTitle:[GDLocalizableClass getStringForKey:@"call.reject"] forState:UIControlStateNormal];
    ...
    [_answerButton setTitle:[GDLocalizableClass getStringForKey:@"call.answer"] forState:UIControlStateNormal];
    ...
    [_hangupButton setTitle:[GDLocalizableClass getStringForKey:@"call.hangup"] forState:UIControlStateNormal];
...
- (void)_initializeCamera
{
    ...
    if(!error){
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:[GDLocalizableClass getStringForKey:@"error"] message:error.localizedFailureReason delegate:nil cancelButtonTitle:[GDLocalizableClass getStringForKey:@"ok"]  otherButtonTitles:nil, nil];
        [alertView show];
    }
...
- (void)_reloadPropertyData
{
    ...
    _sizeLabel.text = [NSString stringWithFormat:@"%@%i/%i", [GDLocalizableClass getStringForKey:@"call.videoSize"], [callManager getVideoWidth], [callManager getVideoHeight]];
    _timedelayLabel.text = [NSString stringWithFormat:@"%@%i", [GDLocalizableClass getStringForKey:@"call.videoTimedelay"], [callManager getVideoTimedelay]];
    _framerateLabel.text = [NSString stringWithFormat:@"%@%i", [GDLocalizableClass getStringForKey:@"call.videoFramerate"], [callManager getVideoFramerate]];
    _lostcntLabel.text = [NSString stringWithFormat:@"%@%i", [GDLocalizableClass getStringForKey:@"call.videoLostcnt"], [callManager getVideoLostcnt]];
    _localBitrateLabel.text = [NSString stringWithFormat:@"%@%i",[GDLocalizableClass getStringForKey:@"call.videoLocalBitrate"], [callManager getVideoLocalBitrate]];
    _remoteBitrateLabel.text = [NSString stringWithFormat:@"%@%i", [GDLocalizableClass getStringForKey:@"call.videoRemoteBitrate"], [callManager getVideoRemoteBitrate]];
}
                           ...
                           error:(EMError *)error
{
    ...
    [self _stopRing];
    if(error){
        _statusLabel.text = [GDLocalizableClass getStringForKey:@"call.connectFailed"];
        [self _insertMessageWithStr:[GDLocalizableClass getStringForKey:@"call.failed"]];
        
        UIAlertView *errorAlert = [[UIAlertView alloc] initWithTitle:[GDLocalizableClass getStringForKey:@"error"] message:error.description delegate:self cancelButtonTitle:[GDLocalizableClass getStringForKey:@"ok" ] otherButtonTitles:nil, nil];
    ...
    if (callSession.status == eCallSessionStatusDisconnected) {
        _statusLabel.text =[GDLocalizableClass getStringForKey:@"call.suspended"] ;
        NSString *str =[GDLocalizableClass getStringForKey:@"call.over"];
        if(_timeLength == 0)
        {
            if (reason == eCallReasonHangup) {
                str = [GDLocalizableClass getStringForKey:@"call.cancel"];
            }
            else if (reason == eCallReasonReject){
                str = [GDLocalizableClass getStringForKey:@"call.rejected"];
            }
            else if (reason == eCallReasonBusy){
                str = [GDLocalizableClass getStringForKey:@"call.in"];
            }
        }
    ...
    else if (callSession.status == eCallSessionStatusAccepted)
    {
        if (callSession.connectType == eCallConnectTypeRelay) {
            _statusLabel.text = [GDLocalizableClass getStringForKey:@"call.speak.relay"];
        }
        else if (callSession.connectType == eCallConnectTypeDirect){
            _statusLabel.text = [GDLocalizableClass getStringForKey:@"call.speak.direct"];
        }
        else{
            _statusLabel.text = [GDLocalizableClass getStringForKey:@"call.speak"];
        }
        _timeLength = 0;
...
- (void)rejectAction
{
    [_timeTimer invalidate];
    [self _stopRing];
    [self showHint:[GDLocalizableClass getStringForKey:@"call.rejected"]];
...
- (void)answerAction
{
    [self showHint:[GDLocalizableClass getStringForKey:@"call.init"]];
    [self _stopRing];
    AVAudioSession *audioSession = [AVAudioSession sharedInstance];
...
- (void)hangupAction
{
    ...
    [_propertyTimer invalidate];
    [self _stopRing];
    [self showHint:[GDLocalizableClass getStringForKey:@"call.dealloc"]];
    AVAudioSession *audioSession = [AVAudioSession sharedInstance];
...
+ (BOOL)canVideo
{
    if([[[UIDevice currentDevice] systemVersion] compare:@"7.0"] != NSOrderedAscending){
        if(!([AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo] == AVAuthorizationStatusAuthorized)){\
            UIAlertView * alt = [[UIAlertView alloc] initWithTitle:[GDLocalizableClass getStringForKey: @"setting.cameraNoAuthority"]message:[GDLocalizableClass getStringForKey:@"setting.cameraAuthority"] delegate:self cancelButtonTitle:nil otherButtonTitles:[GDLocalizableClass getStringForKey:@"ok" ] , nil];
            [alt show];
            return NO;
        }
    }
