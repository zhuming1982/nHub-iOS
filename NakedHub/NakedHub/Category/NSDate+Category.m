...
-(NSString *)formattedTime{
    ...
    if (!hasAMPM) {
        ...
        if (hour <= 24 && hour >= 0) {
         ...
         else if (hour <144  && hour >= 24){
             // 显示为  星期几
        dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"E HH:mm"];
         }
        else{
//           显示为 月/年/日
...
//     NSString *tempStr = [dateFormatter stringFromDate:self];
        ret = [NSString stringWithFormat:@"%@",[GDLocalizableClass getStringForKey:@"Yesterday"]];
    }
    else{
        ret = [dateFormatter stringFromDate:self];
    }
