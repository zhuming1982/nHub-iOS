...
- (void)setup {
	...
	_tokenField = [[TITokenField alloc] initWithFrame:CGRectMake(0, 7, self.bounds.size.width, 42)];
	...
	_separator = [[UIView alloc] initWithFrame:CGRectMake(0, tokenFieldBottom + 7, self.bounds.size.width, 0.5)];
	[_separator setBackgroundColor:[UIColor colorWithRed:208.0/255.0 green:208.0/255.0 blue:212.0/255.0 alpha:1.0]];
	...
	{
        // tokenFieldBottom
		_resultsTable = [[UITableView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_separator.frame), self.bounds.size.width, 10)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [mixPanel track:@"Message_newMessage_listClick" properties:logInDic];
	id representedObject = [_resultsArray objectAtIndex:indexPath.row];
    ...
    token.font = [UIFont fontWithName:@"Avenir-Medium" size:14];
    token.tintColor = [UIColor colorWithRed:38.0/255.0 green:171.0/255.0 blue:240.0/255.0 alpha:1.0];
...
- (void)tokenFieldFrameWillChange:(TITokenField *)field {
	...
	CGFloat tokenFieldBottom = CGRectGetMaxY(_tokenField.frame);
	[_separator setFrame:((CGRect){{_separator.frame.origin.x, tokenFieldBottom + 7}, _separator.bounds.size})];
	[_resultsTable setFrame:((CGRect){{_resultsTable.frame.origin.x, (tokenFieldBottom + 7)}, _resultsTable.bounds.size})];
...
- (void)setup {
	...
	[self setPromptText:[GDLocalizableClass getStringForKey:@"To:"]];
...
- (CGFloat)layoutTokensInternal {
	...
	CGFloat lineHeight = ceilf(self.font.lineHeight) + topMargin + 6;
...
//        label.backgroundColor = [UIColor blackColor];
        ...
        [label setText:text];
        [label setFont:[UIFont fontWithName:@"Avenir-book" size:(self.font.pointSize + 1)]];
        [view addSubview:label];
        
        UILabel * lineLabel = [[UILabel alloc] init];
        [lineLabel setBackgroundColor:[UIColor colorWithRed:208.0/255.0 green:208.0/255.0 blue:212.0/255.0 alpha:1.0]];
        [view addSubview:lineLabel];
        
        NSString *language = [[GDLocalizableClass userLanguage] substringWithRange:NSMakeRange(0, 2)];
        if ([language isEqualToString:@"en"]) {
            view.frame = CGRectMake(0, 0, 34, 20);
            label.frame = CGRectMake(2, 0, 21, 20);
            lineLabel.frame = CGRectMake(30.5, 0, 0.5, 20);
        }
        if ([language isEqualToString:@"zh"]) {
            view.frame = CGRectMake(0, 0, 63, 20);
            label.frame = CGRectMake(2, 0, 50, 20);
            lineLabel.frame = CGRectMake(59.5, 0, 0.5, 20);
        }
...
//		CGFloat components[8] = {red + 0.2, green + 0.2, blue + 0.2, alpha, red, green, blue, 1.0};
...
//		CGGradientRelease(gradient);
    ...
    CGFloat highlightedComp[8] = {red, green, blue, 0.35, red, green, blue, 0.5};
